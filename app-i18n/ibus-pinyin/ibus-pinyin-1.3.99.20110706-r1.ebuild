...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="boost lua nls opencc libpinyin"

RDEPEND=">=app-i18n/ibus-1.3.99
	libpinyin? ( dev-libs/libpinyin	)
	sys-apps/util-linux
	boost? (
		>=dev-libs/boost-1.39
...
src_prepare() {
	if use libpinyin ; then
		epatch "${FILESDIR}/${PN}-libpinyin-integration.patch"
		epatch "${FILESDIR}/${PN}-xx-icon-symbol.patch"
	fi
